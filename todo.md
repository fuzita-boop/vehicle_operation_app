...
## Phase 40: 日次記録の旧版操作感を復元
- [x] 旧版の帰着入力フロー（ホーム通知→帰着フォーム自動展開・スクロール）との差分を確認
- [x] 日次記録画面で未完了記録をオレンジ／黄色で視覚的に区別
- [x] 帰着ボタンで専用フォームへ移動・対象記録の出発情報を表示・自動スクロール
- [x] ホームの帰着未入力バナーから対象帰着フォームを自動展開（対象記録IDをURLへ指定）
- [x] 帰着保存後のホーム復帰と操作フローをテスト（通常操作は日次画面に留まり、ホーム経由はホームへ復帰）
- [x] 帰着対象選択のユニットテストを追加（全4件パス）

## Phase 41: 月次レポートの旧版デザインを復元
- [x] 旧版と現行PWA版の月次レポート画面・印刷レイアウトの差分を確認
- [x] 運転者・車両・対象期間・集計・一覧テーブルの画面デザインを旧版に合わせて復元
- [x] 旧版と同じ印刷／PDF保存用A4レイアウトを復元（ガソリン代欄は印刷時のみ表示）
- [x] サブパス配下での月次レポート表示・印刷対象を検証

## Phase 42: 日次記録一覧の旧版デザイン・編集機能を復元
- [x] 旧版と現行PWA版の日次記録一覧・編集操作の差分を確認
- [x] 記録一覧のカード、未完了バッジ、距離・稼働件数・編集操作を旧版に近い見え方へ復元
- [x] 記録日・出発／帰着時刻・走行距離・稼働件数を編集できるフォームを実装
- [x] 編集の保存・取消・入力検証・一覧即時更新を実装
- [x] 編集フローと未完了記録の編集をテスト（全5件パス）

## Phase 43: 参照画像に合わせた日次記録カード表示
- [x] 通常記録を上段（日付・出発・終了）／下段（出発距離・終了距離・走行距離）の6項目カードに変更
- [x] 編集・削除ボタンをカード右側に固定し、モバイルでも押しやすくする
- [x] 帰着未入力をオレンジ左枠・未入力文字・帰着ボタンで強調
- [x] 稼働件数の表示をカードの情報配置に合わせて整理
- [x] iPhone幅を想定した3列グリッドと操作ボタン配置を確認

## Phase 44: ホーム画面下部の表示崩れ修正
- [x] 公開版・開発版のホーム下部を確認（バックアップ・復元欄の崩れは確認されず）
- [x] ユーザー画像で問題箇所が日次記録カードであることを特定し、Phase 45へ対応を移管
- [x] ホーム画面下部の追加修正は不要と判断

## Phase 45: iPhone幅での日次記録カード重なり修正
- [x] 通常・未完了カードの6項目と操作ボタンが重なる原因を分析
- [x] モバイルでは情報グリッドと操作ボタンを上下に分け、文字・数字の重なりを解消
- [x] iPhone幅で日付・時刻・距離・未入力表示・帰着ボタンが読めることを検証

## Phase 46: 帰着ボタンのフォーム自動展開修正
- [x] 帰着ボタン押下時の対象選択・フォーム状態・スクロール処理を調査
- [x] 対象の帰着入力フォームを確実に展開して表示位置へ移動するよう修正（描画後の二重スクロール＋フォールバック）
- [x] 通常の帰着ボタンとホーム経由の帰着導線をテスト（通常ボタンでフォーム展開・自動スクロールを確認）

## Phase 47: iPhone幅でのフォーム・ボタン幅統一
...
## Phase 51: 標準日付・時刻ピッカーの復元とiPhone幅固定
- [x] 記録日・出発／終了時刻を標準のdate・timeピッカーへ戻す
- [x] iPhone Safariでは入力ラッパーで幅を固定し、ネイティブ入力のカード外はみ出しを防止
- [x] 標準ピッカーの操作層がカード内に収まり、幅がラッパーを超えないことを実測確認

## Phase 52: 仕様変更の事前承認運用
- [x] 標準日付・時刻ピッカーを維持した固定幅ラッパー修正を最終確認・保存（標準ピッカーのユニットテスト2件を追加）
- [x] 以後の既存操作・表示・入力方式の変更は、実装前に影響を説明してユーザー承認を得る

## Phase 53: 出発・終了時刻の現在時刻自動入力
- [x] JST基準の現在時刻をHH:MM形式で取得するユーティリティを追加
- [x] 出発フォーム開始時と出発保存後に出発時間を現在時刻で自動入力
- [x] 帰着フォーム開始時に終了時間を現在時刻で自動入力
- [x] 標準時刻ピッカーでの手動変更・保存をテスト（全8件パス）

## Phase 54: 月次レポートのPDF保存・印刷ボタン修正
...
## Phase 58: iPhone入力セルの行めり込み修正
- [x] 対象をPDF表のセル内文字配置と特定（入力セル修正は対象外としてPhase 59へ移管）
- [x] 日付・時刻ピッカーを含む日次フォームの変更は行わない
- [x] iPhone入力フォームの追加変更は不要と確認

## Phase 59: PDF表レイアウト・出力ファイル名修正
- [x] PDF表のセル内の文字が下行へめり込む原因を調査（行高・縦位置未指定）
