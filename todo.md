...
### アーカイブ済み候補（旧サーバー版・現行PWAの対象外）
- 暗黒モード対応（任意機能。ライトモード固定のまま運用）
...
### アーカイブ済み候補（旧サーバー版・Phase 39で再設計済み）
- 日次記録の入力・保存・表示テスト（現行PWAではUI通し確認とIndexedDB単体テストを実施済み）
- 月次レポート生成テスト（現行PWAでは公開URL・サブパス表示を確認済み）
- モバイルレスポンシブテスト（追加要件が発生した際に現行PWAを対象として実施）

## Phase 7: 完成したアプリをユーザーに提供

### アーカイブ済み候補（旧サーバー版。現行PWAはPhase 39で公開・検証・納品）
- 最終確認テスト
- チェックポイント作成
- ユーザーへの提供
...
## Phase 39: 完全ローカルPWA・GitHub Pages移行
- [x] 現行データのバックアップ形式と一度だけのIndexedDB取込手順を実装
- [x] IndexedDBの運転者・車両・サイクル・日次記録データ層を実装
- [x] 全画面をローカルデータ層へ移行し、Manus認証・tRPC・サーバーDB・外部API依存を削除
- [x] JSONバックアップのエクスポートとインポート（復元）画面を追加
- [x] 印刷・PDF代替をブラウザ印刷に統一し、ローカルデータからA4レイアウトを生成
- [x] Service Worker・Web App Manifest・アプリアイコン・オフライン起動対応を追加
- [x] GitHub Pagesのサブパス対応Vite設定とGitHub Actionsデプロイワークフローを追加
...
- [x] GitHub Pagesで公開（GitHub Actionsデプロイ成功・公開URLでホームと月次レポートを確認）
- [x] 承認済み：GitHubリポジトリを公開へ変更し、GitHub Pagesを有効化
