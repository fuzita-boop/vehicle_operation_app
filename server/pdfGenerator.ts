...
export async function generateMonthlyReportPdf(opts: PdfReportOptions): Promise<Buffer> {
  ...
  return new Promise((resolve, reject) => {
    ...
    // ===== テーブル =====
    // 列定義（印刷HTMLと同じ比率: 16/14/14/19/19/18）
    const totalParts = 16 + 14 + 14 + 19 + 19 + 18;
    const colWidths = [16, 14, 14, 19, 19, 18].map(p => Math.floor(PW * p / totalParts));
    // 端数を最後の列に加算
    const sumW = colWidths.reduce((a, b) => a + b, 0);
    colWidths[5] += PW - sumW;

    const colHeaders = ["日付", "出発時間", "終了時間", "出発時距離(km)", "終了時距離(km)", "走行距離(km)"];
    const rowH = 16;
    ...
    opts.records.forEach((rec, idx) => {
      ...
      const aligns: ("left" | "center" | "right")[] = ["left", "center", "center", "right", "right", "right"];
      const colors = cells.map((_, i) => (isIncomplete && i >= 2) ? "#b45309" : "#000000");
